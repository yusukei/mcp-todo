    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Library className="w-6 h-6 text-accent-600 dark:text-accent-400" />
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {sites.map((site) => (
              <Link
                key={site.id}
                to={`/docsites/${site.id}`}
                className="block p-5 bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-accent-300 dark:hover:border-accent-600 hover:shadow-md transition-all"
              >
                <div className="flex items-start gap-3">
                  <Library className="w-5 h-5 text-accent-500 dark:text-accent-400 mt-0.5 flex-shrink-0" />
