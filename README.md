...
### 認証
- パスキー（WebAuthn）対応 — 生体認証やセキュリティキーでパスワードレスログイン
- パスキー登録後にパスワード認証を無効化可能
- Google OAuth（許可メールリストで制御）

### 管理機能
- ユーザ管理・Google OAuth メール許可リスト
- ユーザごとの API キー管理（アカウント設定から発行）
- API キー認証時にユーザーの有効性もチェック（ユーザー無効化で自動失効）
...
| `WEBAUTHN_RP_ID` | パスキーの RP ID（本番ドメイン名、例: `example.com`） |
| `WEBAUTHN_ORIGIN` | パスキーの Origin（例: `https://example.com`） |
...
API キーは Web UI のアカウント設定（`/settings`）で発行。
...
| 対象 | 方式 |
|------|------|
| 管理者 | メール/パスワード or パスキー → JWT (アクセス60分 + リフレッシュ7日) |
| 一般ユーザ | Google OAuth → `allowed_emails` に事前登録が必要 |
| MCP / API | `X-API-Key` ヘッダ → キー所有者のユーザー有効性も検証 |

### パスキー（WebAuthn）

管理者ユーザーはアカウント設定（`/settings`）からパスキーを登録できます。パスキー登録後、パスワード認証を無効化してパスキーのみでログインすることも可能です。

設定に必要な環境変数:

| 変数 | デフォルト | 説明 |
|------|-----------|------|
| `WEBAUTHN_RP_ID` | `localhost` | Relying Party ID（ドメイン名） |
| `WEBAUTHN_RP_NAME` | `MCP Todo` | Relying Party 表示名 |
| `WEBAUTHN_ORIGIN` | `http://localhost:3000` | ブラウザの Origin |
